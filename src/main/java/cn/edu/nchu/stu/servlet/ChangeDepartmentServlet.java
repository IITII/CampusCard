...
@WebServlet(name = "ChangeDepartmentServlet", displayName = "ChangeDepartment", urlPatterns = "/change_department.do")
        ...
        if (user != null) {
            if (user.getType() == User.ADMINISTRATOR) {
                ...
                if (type != 0 && name != null && !name.trim().isEmpty() && contact != null && !contact.trim().isEmpty()) {
                    dao.updateDepartmentById(id, name.trim(), contact.trim(), type);
